...
stock CObject_Load()
{
	new object, count, tmpBuf[255], Float:fPos[6], modelid, mt_idx, mt_modelid, mt_txdname[32], mt_texture[32],
	...
	CMySQL_Query("SELECT modelid,fX,fY,fZ,fRX,fRY,fRZ,mt_idx,mt_modelid,mt_texture,mt_color,mtxt_idx,mtxt_text,mtxt_msize,mtxt_font,mtxt_fsize,mtxt_fbold,mtxt_fcolor,mtxt_fbcolor,mtxt_falign,vw,interior FROM objects WHERE spawn=1", -1);
	mysql_store_result();
	while(mysql_fetch_row(tmpBuf, "|"))
	{
		if(sscanf(tmpBuf, "p<|>dffffffdds[32]s[32]dds[64]ds[18]ddxxdd",
			modelid, fPos[0], fPos[1], fPos[2], fPos[3], fPos[4], fPos[5],
		...
		object=CreateDynamicObject(modelid, fPos[0], fPos[1], fPos[2], fPos[3], fPos[4], fPos[5], vw, interior, _, 300);
		if(mtxt_idx != -1)
		{
